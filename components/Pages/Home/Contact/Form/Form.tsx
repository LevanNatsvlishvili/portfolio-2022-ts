...
function Contact(props: { shouldDisplay: boolean }) {
  ...
  const [form, setForm] = useState<CleanForm>(new CleanForm());
  ...
  const handleSubmit = (e: React.ChangeEvent<HTMLInputElement>): void => {
    e.preventDefault();
    if (!form.mail || form.message.length < 10) {
      setError(true);
      return;
    }
    if (formRef.current) {
      ...
      >
        ...
        <input
          ...
          className={`bg-[#141414] my-2 rounded-3xl w-full py-2 text-sm px-4 outline-none w-full 
            ${error && form.mail.length < 5 && 'error'}`}
          placeholder="Email"
          required
          value={form.mail}
          name="mail"
        ...
        <textarea
          ...
          ${error && form.message.length < 10 && 'error'} `}
          placeholder="Message"
          required
          value={form.message}
          name="message"
